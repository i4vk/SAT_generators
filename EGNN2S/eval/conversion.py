...
def main():
    ...
    if action == "sat2fg":
        for filename in os.listdir(source_path):
            ...
            fg_filename = filename.split(".")[0] + "_fg_edge_list"
            FG = sat_to_factor_graph(source_path + "/" + filename)
            save_graph_list(FG, "{}/{}".format(store_dir, fg_filename))
    elif action == "fg2sat":
        graphs = load_graphs(source_path)
        benchmark_name = os.path.basename(source_path).split('.')[0]
        for i, graph in enumerate(graphs):
            f = open(f'par_splits_{args.data_name}/{i}.txt')
            par_split = int(f.read())
            FG_to_sat(graph, "{}/{}_{}.cnf".format(store_dir, benchmark_name, i), par_split)
    else:
...
def make_arguments():
    ...
    parser.add_argument("--action", "-a", type=str, default="sat2fg", help="sat2fg/fg2sat")
    parser.add_argument('--data_name', dest='data_name', default='fuhs', help='Family name to use')
    return parser
...
def FG_to_sat(graph, save_name, par_split):
    nodes = list(graph.nodes())
    assert(0 in nodes)
    #num_var = min(list(graph.neighbors(0)))
    num_var = par_split
    #print(coloreado_voraz(graph))
    # print(num_var)
    ...
    for node in nodes:
        if (node >= num_var):
            ...
            assert(len(neighbors) > 0)
            for lit in neighbors:
                if graph.edges[lit, node]['features'] == [1,0]:
                    clause += "{} ".format(lit + 1)
                elif graph.edges[lit, node]['features'] == [0,1]:
                    clause += "{} ".format(-(lit + 1))
            clause += "0\n"
            clauses.append(clause)
            
    with open(save_name, 'w') as out_file:
        out_file.write("c generated by G2SAT Factor-Graph\n")
        out_file.write("p cnf {} {}\n".format(num_var, len(clauses)))
        for clause in clauses:
            out_file.write(clause)
    return
...
def sat_to_factor_graph(source):
    cnf = open(source)
    content = cnf.readlines()
    while content[0].split()[0] == 'c':
        content = content[1:]
    ...
    FG = nx.Graph()
    FG.add_nodes_from(range(num_vars + num_clause + 1)[1:])
    preprocess_FG(formula, FG, num_vars)
    return FG
...
def preprocess_FG(formula, FG, num_vars):
    """
    Builds Factor Graph
    """
    for cn in range(len(formula)):
        for var in formula[cn]:
            if var > 0:
                FG.add_edge(var, cn+num_vars+1, features=[1,0])
            elif var < 0:
                FG.add_edge(abs(var), cn+num_vars+1, features=[0,1])
