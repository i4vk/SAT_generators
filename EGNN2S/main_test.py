...
print(args)
np.random.seed(123)
args.name = '{}_{}_{}_{}_pre{}_drop{}_yield{}_{}'.format(
    args.data_name, args.model, args.layer_num, args.hidden_dim, args.feature_pre, args.dropout, args.yield_prob, args.comment)
...
if args.recompute_template or not os.path.isfile(template_name):
    graphs_train, nodes_par1s_train, nodes_par2s_train = load_graphs_fg(data_dir=f'dataset/{args.data_name}/', stats_dir='dataset/')
    save_graph_list(graphs_train,train_name, has_par=True,nodes_par1_list=nodes_par1s_train,nodes_par2_list=nodes_par2s_train)
    print('Train graphs saved!', len(graphs_train), flush=True)
    ...
    print('Num {}, Node {} {} {}, Edge {} {} {}'.format(
        ...
        max(edge_nums), sum(edge_nums) / len(edge_nums)), flush=True)
    ...
    print('Template saved!', flush=True)
else:
    graph_templates, nodes_par1s, nodes_par2s = load_graph_list(template_name, has_par=True)
    print('Template loaded!')


print('Template num', len(graph_templates), flush=True)


# load stats
with open('dataset/' + 'fg_stats.csv') as csvfile:
...
#generator_list = generator_list[80:90]
# generator_list = [generator_list[2], generator_list[7]]
...
input_dim = 2 # 2 node types
edge_features_dim = 2 # 2 edge types
